...
class RunDQN:
    ...
    def train_model(self):
        ...
        while True:
            ...
            action = self.dqn.act([runner_utils.other_to_torch(s[np.newaxis], self.device) for s in state], step)
    ...
    def collect_data(self, num_exp, eps, save_path):
        ...
        for i in range(num_exp):
            ...
            action = self.dqn.act(
                [runner_utils.other_to_torch(s[np.newaxis], self.device) for s in state], 0
            )
            with torch.no_grad():
                qs = self.dqn(
                    [runner_utils.other_to_torch(s[np.newaxis], self.device) for s in state]
                ).cpu().numpy()[0, :]

            abstract_action = self.env.get_abstract_action(action)
            next_state, reward, done, _ = self.env.step(action)

            dataset.add(Constants.STATES, state[0])
            dataset.add(Constants.HAND_STATES, state[1])
            ...
            dataset.add(Constants.NEXT_STATES, next_state[0])
            dataset.add(Constants.NEXT_HAND_STATES, next_state[1])
        ...
        dataset = dataset.to_array_dataset({
            ...
            Constants.DONES: np.bool, Constants.HAND_STATES: np.float32, Constants.NEXT_HAND_STATES: np.float32
        })
        dataset.metadata = {
    ...
    def label_data(self, states, hand_states, actions=None):
        ...
        for i in range(num):
            ...
            batch_states = runner_utils.other_to_torch(batch_states, self.device)

            batch_hand_states = hand_states[i * self.batch_size: (i + 1) * self.batch_size]
            batch_hand_states = runner_utils.other_to_torch(batch_hand_states, self.device)

            with torch.no_grad():
                qs = self.dqn([batch_states, batch_hand_states]).cpu().numpy()
    ...
    def demonstrate_dqn(self):
        ...
        while True:
            ...
            print(action)
            print("hand: {:s}".format(str(state[1])))
            plt.imshow(FruitsSeq.state_to_image(state[0]))
            plt.show()
    ...
    def learn_step_(self, opt, step):
        ...
        states = [runner_utils.other_to_torch(np.array([s[i] for s in states], dtype=np.float32), self.device) for i in [0, 1]]
        ...
        next_states = [runner_utils.other_to_torch(np.array([s[i] for s in next_states], dtype=np.float32), self.device) for i in [0, 1]]
    ...
    def evaluate_(self):
        ...
        for i in range(self.NUM_EVAL_EPISODES):
            ...
            while True:
                ...
                action = self.dqn.act(
                    [runner_utils.other_to_torch(s[np.newaxis], self.device) for s in state], 0, evaluation=True
                )
    ...
    def build_env_(self):
        ...
        self.env = FruitsSeq(num_fruits=self.num_fruits, max_steps=30)
    ...
    def build_models_(self):
        ...
        if self.side_transfer:
            build_dqn_fc = self.build_dqn_side_transfer_
        else:
            build_dqn_fc = self.build_dqn_
    ...
    def build_dqn_side_transfer_(self):
