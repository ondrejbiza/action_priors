...
from ....run.online.fruits.RunDQNAAF import RunDQNAAF
...
def build_aaf_model(model_config, logger, device):
    ...
    reshape_encoder = FlatStatesOneHotActions(num_actions)

    fc_config = {
        Constants.INPUT_SIZE: int(np.prod(input_size)) + num_actions,
